...
async def post(repo_url: str, regenerate: bool = False):
    ...
    try:
        ...
        if not exists or regenerate:
            ...
            try:
                # Only generate embeddings if the provider supports it
                if LLMProviderManager.supports_embeddings() and hasattr(llm_client, "embeddings"):
                    ...
                    embedding = llm_client.embeddings.create(input=truncated_context, model=embedding_model).data[0].embedding
                    embedding_json = json.dumps(embedding)
                else:
                    logging.info(f"Embeddings not supported for provider: {LLMProviderManager.get_provider()}")
                    embedding_json = None
                ...
                devcontainer_dict = new_devcontainer.model_dump(exclude_unset=True, exclude={'id'})
                
                # Ensure id is not in the dictionary at all
                if 'id' in devcontainer_dict:
                    del devcontainer_dict['id']
...
if check_env_vars():
    ...
    # Get model from env or use smart default selection
    model_from_env = os.getenv("MODEL")
    model = LLMProviderManager.get_default_model()  # This handles validation and auto-selection
    
    logging.info("=" * 60)
    logging.info("LLM Configuration:")
    logging.info(f"  Provider: {provider}")
    if model_from_env:
        if model == model_from_env:
            logging.info(f"  Model: {model} (from .env)")
        else:
            logging.info(f"  Model: {model} (auto-selected, .env had incompatible '{model_from_env}')")
    else:
        logging.info(f"  Model: {model} (auto-selected default)")
    logging.info("=" * 60)
    
    llm_client = setup_llm_client()
    instructor_client = setup_instructor(llm_client)
else:
    logging.error("Environment variables check failed. Please configure them properly.")
    exit(1)
